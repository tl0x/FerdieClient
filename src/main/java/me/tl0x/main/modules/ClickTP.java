...
public class ClickTP extends Module {
    public ClickTP() {super("ClickTP","TPs to where you right click");}
    int maxDistance = 5;
        ...
        if (enabled) {
            ...
            if (MinecraftClient.getInstance().options.keyUse.isPressed()) {
                ...
                if (hitResult.getType() == HitResult.Type.BLOCK) {
                    ...
                    if (MinecraftClient.getInstance().world.getBlockState(pos).onUse(MinecraftClient.getInstance().world, MinecraftClient.getInstance().player, Hand.MAIN_HAND, (BlockHitResult) hitResult) != ActionResult.PASS) return;
